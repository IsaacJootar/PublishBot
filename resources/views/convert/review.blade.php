<x-app-layout>
    ...
    <div style="max-width:800px;">
        ...
        <div class="pai-card" style="margin-bottom:1.25rem;border-top:3px solid #F59E0B;">
            <div style="display:flex;align-items:flex-start;gap:0.75rem;flex-wrap:wrap;">
                ...
                <div style="flex:1;">
                    ...
                    <p style="margin:0.2rem 0 0;font-size:0.78rem;color:#9B93B0;">Original file: {{ $run->original_filename }}</p>
                </div>
            </div>

            @if($run->progress_note)
            <div id="progress-banner" style="margin-top:0.85rem;background:#EFF6FF;color:#1D4ED8;border-radius:8px;padding:0.55rem 0.8rem;font-size:0.78rem;font-weight:600;">
        ...
        <div class="pai-card" style="text-align:center;padding:2.5rem 1.5rem;margin-bottom:1.25rem;">
            ...
            <p id="status-text" style="font-size:0.82rem;color:#9B93B0;margin:0;">{{ $run->progress_note ?? "Processing sections..." }}</p>
        </div>
        <script>
        (function() {
        ...
        <div class="pai-card" style="margin-bottom:1.25rem;">
            <div style="display:flex;align-items:center;justify-content:space-between;gap:0.75rem;margin-bottom:0.85rem;flex-wrap:wrap;">
                <div>
                    ...
                    <p style="margin:0.15rem 0 0;font-size:0.78rem;color:#9B93B0;">Edit anything here before approving. Your original is preserved.</p>
                </div>
            </div>

            <form method="POST" action="{{ route("convert.approve", $run) }}">
                @csrf
                <textarea name="cleaned_content" rows="30" class="pai-input" style="font-family:"Courier New",monospace;font-size:0.82rem;line-height:1.7;resize:vertical;margin-bottom:1rem;">{{ $run->cleaned_content }}</textarea>
                ...
                <div style="display:flex;gap:0.75rem;flex-wrap:wrap;align-items:center;">
                    ...
                    <p style="font-size:0.75rem;color:#9B93B0;margin:0;">Next: KDP listing generation</p>
                </div>
            </form>
        </div>

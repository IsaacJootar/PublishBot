...
<style>
    ...
    .cover h1 { font-size:32pt; font-weight:700; color:#fff; margin:0 0 16px; line-height:1.2; }
    ...
    .section-hdr { background:#1A0D33; color:#fff; padding:14px 20px; border-radius:6px; margin-bottom:22px; }
    ...
    .section pre { font-family:DejaVu Sans Mono,monospace; font-size:8.5pt; background:#F5F4FF; padding:10px; border-radius:4px; border-left:3px solid #6C3CE1; white-space:pre-wrap; word-wrap:break-word; margin:0 0 10px; }
    footer { position:fixed; bottom:-50px; left:0; right:0; text-align:center; font-size:8pt; color:#999; }
</style>
</head>
<body>
<footer>{{ $title }} · 90-Day Content Calendar System</footer>
<div class="cover">
    <div class="badge">Content Calendar System</div>
    <h1>{{ $title }}</h1>
...
<div class="section">
    ...
    @foreach(preg_split("/\n\s*\n/", trim($s['body'])) as $para)
        ...
        @if($p !== '')
            @if(str_starts_with($p, '|') || str_starts_with($p, 'Day ') || str_starts_with($p, 'POST '))
                <pre>{{ $p }}</pre>
            @else
                <p>{!! nl2br(e($p)) !!}</p>
            @endif
